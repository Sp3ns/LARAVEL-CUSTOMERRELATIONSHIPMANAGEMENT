...
<div class="max-w-3xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('customers.show', $customer) }}" class="text-sm text-gray-500 hover:text-indigo-600 flex items-center gap-1">
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-6">{{ $customer->full_name }}</h2>
        ...
        <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="address" rows="2" class="w-full rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('address', $customer->address) }}</textarea>
                </div>
                <div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                <a href="{{ route('customers.show', $customer) }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 rounded-lg hover:bg-gray-100 transition-colors">Cancel</a>
                <button type="submit" class="px-6 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">Update Customer</button>
            </div>
        </form>
    </div>
</div>
